...
public class GestionEvenementController implements Initializable {
    ...
    void Acceuil(MouseEvent event){
        try {
            fxml = FXMLLoader.load(getClass().getResource("/gui/Acceuil/AcceuilScene.fxml"));
    ...
    @FXML
    private void add(ActionEvent event) throws SQLException, WriterException {
         if (controleDeSaisi()){
        ResultSet rs6 = null ; 
        ResultSet rs7 = null ; 
        ResultSet rs8 = null ; 
       ...
       String datef=DF.getValue().format(DateTimeFormatter.ofPattern("yyyy-MM-dd"));
         ...
         initialiserlist(); 
                ...
                RECl.refresh();
        try {
            es.ajouter(new evenement(y,id1,id2,id3,NE.getText(),DESC.getText(),dated,datef,g,f,activite.getText()));
                            afficherEvenement();
